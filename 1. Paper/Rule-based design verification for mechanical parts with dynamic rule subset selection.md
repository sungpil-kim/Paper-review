...
1. 설계 데이터에서 설계 규칙 지식 기반(1)과 설계 매개변수 데이터(2)를 로더한다.
2. 설계 규칙 지식 기반(1)의 데이터를 관리자가 설계 변수(A)와 검증 방정식(B)으로 관리한다.
3. 설계 매개변수 데이터(B)는 컨트롤러가 관리한다.
4. 관리자와 컨트롤러가 설계 변수와 매개변수를 비교하며 변수(문제점)을 식별하고, 변수가 발생 시 사용자에게 
